...
function system_email_source_profile {
    ...
    python3 send_email.py "$(hostname) - The profile was refreshed. - $(date)" "/bin/bash 'source ~/.profile; system_source_profile'"
}
...
function system_emerge_sync {
    sudo emaint -a sync
}

function system_emerge_update {
...
function system_rsync_git_prod {
...
function system_venv_activate_source {
    source ~/.local/bin/venv/bin/activate
}

function system_spice_all {
