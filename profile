...
function apk_setup_ish() {
    ...
    mkdir -p ~/.local/bin/
    apk_upgrade_ish
	# i3status i3status-doc i3wm i3wm-doc i3lock i3lock-doc sshfs
    package_list=( bash dos2unix git git-lfs lynx nano neofetch openrc openssh openssl python3 py3-pip rsync sqlite sudo tmux tree ttf-dejavu vim x11vnc x11vnc-doc xdpyinfo xdpyinfo-doc xf86-video-dummy xorg-server xterm xvfb)
    for i in "${package_list[@]}"
    do
        apk_install $i
    done
